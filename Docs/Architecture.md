...
`FishingRunView` owns the reusable root gameplay prefab and its stable presentation regions: background, top navigation, Catch Rig, current Encounter, run controls, Technique hand, tooltips, transitions, and modals. Its root Canvas scales from 1920 x 1080 with equal width/height matching. The background is one user-supplied complete 1920 x 1080 nautical-chart image with all gameplay UI layered above it. The top navigation uses one complete edge-to-edge frame displayed at 1920 x 80, while region, depth, deck count, icons, and buttons remain independent children. Screen-wide warning, release, and modal feedback remain separate overlays. Major gameplay regions expose independent visibility through `CanvasGroup` state, while tooltip, transition, and modal canvases have deterministic render order above gameplay.
